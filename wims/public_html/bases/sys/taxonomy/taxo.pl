...
my %addrsheet = reverse_dic ("$sheetdir/index/addr"); my $addrsheet=\%addrsheet;
my %titsheet = treate_dict ("$sheetdir/index/title"); my $titsheet=\%titsheet;

my %filetoread=('mathematics'=> 'taxo_modulelist',
'physics'=> 'taxo_modulelist',
'biology'=> 'taxo_modulelist',
'chemistry'=> 'taxo_modulelist',
'informatics' => 'taxo_modulelist',
'ecogestion' => 'taxo_modulelist',
'geoscience' => 'taxo_modulelist',
'clmath' => 'taxo_modulelist');
for my $t (glob("$ddir/*.$LANG")) {
  $t =~ s,$ddir\/,,g;
  my @t_=split('\.',$t); next if !($t_[1] eq $LANG);
  next if $t_[0] eq 'commoncore';
  next if $t_[0] eq 'taxocl';
  taxonomy($t_[0], $filetoread{$t_[0]}, $LANG, '_','_');
}

## impose order in the list
my $cltitle='CL_'; my @list1=();
for my $l ('8','7','6','5','4','3') { push @list1, "$cltitle$l" }
#for my $s ('_O','_Z','_Q','_P','_L','_R','_E','_T','_A','_TR','_S','_F','_I','_D','_M','_V') {
#  for my $l ('_C','_G','_S','_P','_I') {
#    for my $n ('5','4','3') {
#    push @list1, "$cltitle$n" ;
#    push @list1, "$cltitle$n$l" ;
#    push @list1, "$cltitle$n$l$s"}
#  }
#}
taxonomy ('clmath', 'taxo_modulelist','fr', '_','_',@list1);

my $ccsstitle='CCSS.Math.Content_'; my @list=();
for my $l ('K','1','2','3','4','5','6','7','HS') { push @list, "$ccsstitle$l" }
taxonomy ('commoncore', 'taxo_modulelist', $LANG, '_','_',@list);
...
sub taxonomy { my ($taxo, $taxo2, $lang, $sep1, $sep2, @list ) = @_ ;
  ...
  $T .= "</ul>";
  if (!$module) {
    $T .= "<input type=\"checkbox\" name=\"taxon_$taxo\" id=\"empty\" value=\"\"> $joker";
  }
  $T .= "<div class=\"wims_credits\">";
  $T .= " <strong>\$wims_name_credits:</strong> $desctaxo";
  $T .= "</div>";
...
sub one {my ($a, $taxo, $desc, $tit, $ref, $vu)=@_;
  ...
  my $amod = $a;
  if (!$tit->{$a}) { print "warning $a has no title" ; return ""};
  ...
  if (!$module) {
    $T .= "<input type=\"checkbox\" name=\"taxon_$taxo\" id=\"$amod\" value=\"$amod\">"
    ...
    for my $a (@aa) {
      ...
      if (defined($addr{$a})) {
        ...
        if (@exo) {
          ...
          ## liste des exos du module: est-ce utile ?
          #$ref{'text'}{$ligne[0]} .='<ul class="smaller">';
...
## on est sur une feuille ? trouver un bon critere
        if (defined($addrsheet{$a})) {
          if (!$titsheet->{$addrsheet{$a}}) { print "$a\n" ; $titsheet->{$a}=$a};
          $titsheet->{$addrsheet{$a}}=~ s/Evalwims-(.*) ://;
          my $bb=($titsheet->{$addrsheet{$a}}) ? $titsheet->{$addrsheet{$a}} .
          "<span class=\"small hidden\">($b)</span>":$b;
          $ref{'sheet'}{$ligne[0]} .="<li class=\"taxo_module\">\n<span class=\"wims_sheet_item\">$bb</span>"
          . "\n!set wims_ref_class=wims_button\n"
          . "!href target=wims_internal module=adm/sheet\&+job=read\&+sh=$a &rArr;\n
          </li>";
        }
      }
    }
