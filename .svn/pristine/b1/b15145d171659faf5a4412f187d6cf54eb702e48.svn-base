...
!if not_secure iswordof $error
 Solo se aceptan el envío y recepción de datos procedentes de hojas de cálculo en
...
!! login_missing: not find in the ldap for example
   Algunos identificadores de usuario no han podido ser encontrados.
 !exit
!endif
...
!endif
!if too_big=$error
  El número de columnas solicitadas por la hoja de cálculo es demasiado grande
  (superior a $TOO_BIG). Si desea obtener los detalles de los ejercicios,
  hágalo hoja por hoja o examen por examen.
  El archivo a descargar no le entrega toda la información que ha solicitado.
 !exit
!endif
