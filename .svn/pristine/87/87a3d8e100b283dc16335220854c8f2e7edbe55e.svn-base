...
!if prohibited = $error
  $errorhead Su mensaje contiene palabras que no están permitidas. Por ejemplo, no
...
!if open_tag = $error
  ...
  (encerrándolas entre el par de paréntesis \( y \) ).</p>
  !exit
!endif
...
!if $error=not_owner
  $errorhead Solamente el propietario de un foro de discusión está autorizado a configurarlo.
  !set job=list
  !exit
!endif
