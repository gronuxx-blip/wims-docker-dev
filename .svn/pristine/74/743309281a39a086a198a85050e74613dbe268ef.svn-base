...
- récupérer une taxonomie avec des identifiants "logiques" du type
    ...
    Ici `01_02` est bien sûr un fils de `01`.

- la sauver sous les noms `ma_taxo.fr` et `ma_taxo.en` dans le répertoire
`wims/public_html/bases/sys/taxonomy`
(on peut bien sûr mettre d'autres langues mais c'est le minimum, même si tout est en francais).
...
Pour l'instant, quelque chose à changer dans `~/wims/public_html/modules/adm/tool/classification/var.def`
