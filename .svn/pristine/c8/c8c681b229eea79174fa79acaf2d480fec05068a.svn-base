...
!let name_listscoreoption=van de klas,van het rapport
...
!distribute item Activiteit,Kleur,Legenda\
...
!if $job=delpalier
 !let name_delpalier=!nosubst Weet u zeker dat $name_n_palier $palier ( $mem_palier</span> van $name_competence $comp ( $mem_title</span> verwijderd moet worden uit de $name_livret ?
 !exit
!endif

!if $job=delcomp
 !let name_delcomp=!nosubst Weet u zeker dat $name_competence $comp <span class="wims_emph">$mem_title</span> verwijderd moet worden uit $name_livret ?
 !exit
!endif
